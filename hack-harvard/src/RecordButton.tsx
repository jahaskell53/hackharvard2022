...
function RecordButton({isRecording, startRecording}) {
...
//   const [isClicked, setIsClicked] = useState(false);
  ...
  return (<>
    <button className={`shadow-md transition-all rounded-lg font-bold py-2 px-4 rounded text-2xl inline-flex flex items-center gap-2 ${array[isRecording ? 1 : 0].props}`} 
    onClick={(e) => { e.preventDefault();
        startRecording();}}>
        {array[isRecording == "started" ? 1 : 0].icon}
        {array[isRecording == "started"? 1 : 0].name}
</button>
</>);
}
