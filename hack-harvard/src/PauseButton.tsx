...
function PauseButton({isPaused, setIsPaused}) {
...
<path fill-rule="evenodd" d="M4.5 5.653c0-1.426 1.529-2.33 2.779-1.643l11.54 6.348c1.295.712 1.295 2.573 0 3.285L7.28 19.991c-1.25.687-2.779-.217-2.779-1.643V5.653z" clip-rule="evenodd" />
  ...
  return (<>
    <button className={`shadow-md transition-all rounded-lg font-bold py-2 px-4 rounded text-2xl inline-flex flex items-center gap-2 ${array[isPaused ? 1 : 0].props}`} onClick={(e) => { e.preventDefault();
    setIsPaused(!isPaused)}}>
        {array[isPaused ? 1 : 0].icon}
        {array[isPaused ? 1 : 0].name}
</button>
</>);
}
